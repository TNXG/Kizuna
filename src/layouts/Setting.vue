<template>
    <div class="flex items-center justify-center min-h-screen min-w-screen bg-base-200 lg:ml-60">
        <div class="card p-6 max-w-lg mx-auto w-full shadow-2xl bg-base-100">
            <div class="card-body">
                <h1 class="card-title text-2xl font-bold mb-4">设置</h1>
                <form @submit.prevent="saveConfig">
                    <div class="form-control mb-4">
                        <label class="label" for="endpoint">
                            <span class="label-text">服务器端点</span>
                        </label>
                        <input v-model="config.server_config.endpoint" class="input input-bordered w-full" id="endpoint"
                    ...
                    <div class="form-control mb-4">
                        ...
                        <input v-model="config.server_config.token" class="input input-bordered w-full" id="token"
                    ...
                    <div class="form-control mb-4">
                        ...
                        <input v-model.number="config.server_config.report_time" class="input input-bordered w-full"
                            id="report_time" type="number" placeholder="报告时间" />
                    </div>
                    <div v-for="(rule, index) in config.rules" :key="index" class="form-control mb-4">
                        ...
                        <div v-if="!rule.collapsed">
                            ...
                            <input v-model="rule.match_application" class="input input-bordered w-full"
                                :id="'match_application_' + index" type="text" placeholder="匹配应用程序" />
                            <label class="label" :for="'application_' + index">
                                <span class="label-text">替换应用程序</span>
                            </label>
                            <input v-model="rule.replace.application" class="input input-bordered w-full"
                                :id="'application_' + index" type="text" placeholder="替换应用程序" />
                            <label class="label" :for="'description_' + index">
                                <span class="label-text">描述</span>
                            </label>
                            <input v-model="rule.replace.description" class="input input-bordered w-full"
                                :id="'description_' + index" type="text" placeholder="描述" />
                            <div class="flex justify-center">
                                <button type="button" class="btn btn-error mt-2 w-full" @click="removeRule(index)">删除规则</button>
                            </div>
                        </div>
                    </div>
                    <div class="form-control mb-4">
